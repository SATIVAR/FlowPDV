...
import { ChevronLeft, Trash2, PlusCircle, CreditCard, Smartphone, DollarSign } from 'lucide-react';
...
export default function EditOrderPage() {
    ...
    useEffect(() => {
        ...
        if (foundOrder) {
            ...
            const paymentMethodId = predefinedPaymentMethods.find(pm => pm.name === foundOrder.paymentMethod)?.id;
            ...
            form.reset({
                ...
                gatewayName: foundOrder.paymentGateway || '',
                splitPayments: foundOrder.splitPayments?.map(sp => ({
                    methodId: predefinedPaymentMethods.find(pm => pm.name === sp.method)?.id || '',
                    amount: sp.amount,
                })) || [],
                                                ...
                                                >
                                                    {paymentMethods.map((pm) => (
                                                        <FormItem key={pm.id} className="flex-1">
                                                            <FormControl>
                                                                <RadioGroupItem value={pm.id} id={`edit-${pm.id}`} className="peer sr-only" />
                                                            </FormControl>
                                                            <Label
                                                                htmlFor={`edit-${pm.id}`}
